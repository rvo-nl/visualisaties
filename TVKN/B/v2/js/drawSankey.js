...
function drawSankey (sankeyDataInput, config) {
  ...
  d3.select('#' + config.targetDIV).append('svg').style('position', 'relative').attr('id', config.sankeyInstanceID + '_sankeySVGPARENT').attr('width', scrollExtentWidth + 'px').attr('height', scrollExtentHeight + 'px').style('pointer-events', 'none').append('g') // .attr('id', 'sankeySVG').style('pointer-events', 'auto') // scrollExtentWidth
  ...
  sankeyCanvas.append('rect').attr('id', 'delineator_rect_koolstofketen_uit').attr('width', 230).attr('height', 190).attr('x', 290).attr('y', 100).attr('fill', '#DCE6EF').attr('rx', 10).attr('ry', 10).style('stroke', '#BBB').style('stroke-width', '0px').style('opacity', 0)
  ...
  sankeyCanvas.append('rect').attr('id', 'delineator_rect_finaal_overige').attr('width', 230).attr('height', 140).attr('x', 1352).attr('y', 830).attr('fill', '#DCE6EF').attr('rx', 10).attr('ry', 10).style('stroke', '#BBB').style('stroke-width', '0px').style('opacity', 0)
  ...
  sankeyCanvas.append('text').attr('id', 'delineator_text_warmteproductie_bij_finaal_verbruik_uit').attr('x', 315).attr('y', 638).attr('fill', '#666').style('font-weight', 400).style('font-size', '16px').text('LOKAAL').style('opacity', 0)
...
function tick (config) {
  ...
  switch (globalActiveEnergyflowsFilter) {
    ...
    case 'electricity':
      ...
      d3.select('#delineator_rect_koolstofketen_uit').transition().duration(1000).style('opacity', 1).attr('y', 100).attr('height', 190)
      ...
      d3.select('#delineator_rect_koolstofketen_in').transition().duration(1000).style('opacity', 1).attr('height', 130).attr('y', 600)
    ...
    case 'hydrogen':
      ...
      d3.select('#delineator_rect_koolstofketen_in').transition().duration(1000).style('opacity', 1).attr('height', 130).attr('y', 245)
      ...
      break
    case 'heat':
      d3.select('#delineator_text_elektriciteitsketen_uit').transition().duration(1000).style('opacity', 1).attr('y', 395)
      d3.select('#delineator_rect_elektriciteitsketen_uit').transition().duration(1000).style('opacity', 1).attr('height', 150).attr('y', 365)
      ...
      d3.select('#delineator_text_waterstofketen_uit').transition().duration(1000).style('opacity', 1).attr('y', 280)
      d3.select('#delineator_rect_waterstofketen_uit').transition().duration(1000).style('opacity', 1).attr('height', 110).attr('y', 242)
      ...
      d3.select('#delineator_rect_koolstofketen_uit').transition().duration(1000).style('opacity', 1).attr('y', 100).attr('height', 130)
      d3.select('#delineator_text_koolstofketen_uit').transition().duration(1000).style('opacity', 1).attr('y', 138)
    ...
    case 'carbon':
      ...
      d3.select('#delineator_text_waterstofketen_uit').transition().duration(1000).style('opacity', 1).attr('y', 260)
      d3.select('#delineator_rect_waterstofketen_uit').transition().duration(1000).style('opacity', 1).attr('height', 110).attr('y', 222)

      d3.select('#delineator_rect_warmteketen_uit').transition().duration(1000).style('opacity', 1).attr('y', 347)
      d3.select('#delineator_text_warmteketen_uit').transition().duration(1000).style('opacity', 1).attr('y', 379)

      d3.select('#delineator_rect_vlak_conversie').transition().duration(1000).style('opacity', 1).attr('height', 425).attr('y', 550)
...
function updateSankey (json, offsetX, offsetY, fontSize, fontFamily, config) {
  ...
  d3.selectAll('#' + config.sankeyInstanceID + ' .link').style('pointer-events', 'auto').style('cursor', 'pointer')
  d3.selectAll('#' + config.sankeyInstanceID + ' .node').style('pointer-events', 'auto')
  d3.selectAll('#' + config.sankeyInstanceID + ' .node-backdrop-title').style('pointer-events', 'none') // otherwise nodevalue text gets in the way of mouseclick 
  d3.selectAll('#' + config.sankeyInstanceID + ' .node-click-target').style('fill', '#555').style('stroke-width', 0).attr('width', 10).attr('rx', 0).attr('ry', 0).attr('transform', 'translate(-4,0)scale(1.005)')
  // attach id's to link paths
  d3.select('#' + config.sankeyInstanceID + ' .sankey').select('.links').selectAll('.link').select('path').attr('data-value', function (d) {return {value: d.value, color: d.color}}).attr('id', function (d, i) { return 'linkindex_' + d.index}).on('click', function () { drawBarGraph(sankeyDataObjects[globalActiveEnergyflowsSankey.id].links[this.id.slice(10)], config) }).style('opacity', 0.9)
  // attach id's to node rects
  d3.select('#' + config.sankeyInstanceID + ' .sankey').select('.nodes').selectAll('.node').select('.node-click-target').attr('id', function (d, i) {return 'nodeindex_' + d.index}).on('click', function () { nodeVisualisatieSingular(config, sankeyDataObjects[globalActiveEnergyflowsSankey.id].nodes[this.id.slice(10)], sankeyDataObjects[globalActiveEnergyflowsSankey.id], config.scenarios, config.targetDIV) })

  d3.selectAll('#' + config.sankeyInstanceID + ' .link').on('mouseover', function (event, d) {showValueOnHover(d3.select(this)); d3.select(this).style('opacity', 0.8)}).on('mouseout', function (d) {d3.select(this).style('opacity', 1)})
}
