...
function initSankey (config) {
  ...
  function drawSankey (sankeyData, legend, config) {
    ...
    function drawHeader () {
      ...
      titleDiv.style.top = '0px' // Distance from the top of the container
      ...
      versionDiv.style.top = '10px' // Distance from the top of the container
  ...
  function updateSankey (json, offsetX, offsetY, fontSize, fontFamily) {
    ...
    function showValueOnHover (value) {
      ...
      d3.select('#showValueOnHover').html(value._groups[0][0].__data__.source.title + ' → ' + value._groups[0][0].__data__.target.title + ': € ' + formatMillions(parseInt(value._groups[0][0].__data__.value)) + ' miljoen').style('background-color', value._groups[0][0].__data__.color).interrupt().style('opacity', 1)
